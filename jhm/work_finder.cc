...
bool TWorkFinder::ProcessResult(TJobRunner::TResult &result) {
  ...
  TJson job_info(TJson::TObject{
      {"build_info",
       TJson::TObject{{"job",
                       TJson::TObject{{"name", result.Job->GetName()},
  ...
  if(range.first != range.second) {
    for(auto iter = range.first; iter != range.second; ++iter) {
      // Update each job which was waiting on this job's waiting entry. Queue in ReadyJobs if
      // needed.
...
bool TWorkFinder::IsBuildable(TFile *file) { return file->IsSrc() || TryGetProducer(file); }
...
// TODO: Lots of generic helper functions (Esp. dealing with nanosecond timestamps), should be
// factored into library.
...
auto GetTimestampInput(TFile *file) { return *GetTimestampOutput(file); }
...
void TWorkFinder::CacheCheck(TJob *job) {
  ...
  /* For a job to cache-complete, it must meet several requirements
     ...
     4. Every output has a cache file which matches the file. Said cache file has an equal or newer
    timestamp.
     5. Every output's producer job matches this job.

    NOTES:
      - For jobs with unknown outputs, read the set of outputs in from the out file. If we succeed
    in finding them all,
  ...
  // TODO: There will be a lot of redundant stat() calls on input files... Make TFile hold a
  // timestamp which is set
  ...
  // If the input file is in the source tree, then it's timestamp relative to the config doesn't
  // matter.
  ...
  // For all known outputs (There is always at least one), choose one at random and load it's cache
  // file / treat it as
  ...
  try {
    ...
    // Every needs must exist, be done. in_timestamp is the newest of all the needs and the input
    // file.
    for(const string &need_filename :
        ideal_out.Read<vector<string>>({"build_info", "job", "needs"})) {
    ...
    for(const string &filename :
        ideal_out.Read<vector<string>>({"build_info", "job", "anti_needs"})) {
    ...
    // Then in the output loop immediately following this, we ensure that one contains every item in
    // the other.
    ...
    for(const string &output_filename : output_filename_list) {
      ...
      // NOTE: Technically all build info should match exactly. But this should be good enough (and
      // faster).
      // Check the ideal out matches the job
    ...
    // If any of the config files don't contain the requested key(s), then exit / the config must be
    // out of date.
    return;
  }
  ...
  // TODO: Cache finished jobs shouldn't need to be in all (This means that all != finished + ready
  // + waiting);
  All.insert(job);
}
