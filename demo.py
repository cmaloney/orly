...
if __name__ == '__main__':
    ...
    for path in [args.data_dir, args.packages_dir, args.webui_dir, dataset_dir]:
        if not os.path.isdir(path):
            raise RuntimeError(
                      '{0} must be an existing directory.'.format(path))
    # Launch orlyi.
    try:
        orlyi = subprocess.Popen(
                    ['orlyi',
                        '--create=true',
                        '--block_cache_size={0}'.format(args.mem_sim_mb / 4),
                        '--instance_name={0}'.format(args.dataset),
                        '--la',
                        '--le',
                        '--mem_sim=true',
                        '--mem_sim_mb={0}'.format(args.mem_sim_mb),
                        '--package_dir={0}'.format(args.packages_dir),
                        '--page_cache_size={0}'.format(args.mem_sim_mb / 4),
                        '--starting_state=SOLO',
            ...
            if files:
                subprocess.check_output(
                    ['core_import',
                     '--la',
                     '--le',
                     '--import_pattern={0}'.format(os.path.join(root, '*.bin*')),
                     '--num_load_threads=1',
                     '--num_merge_threads=1',
                     '--num_sim_merge={0}'.format(max(8, len(files)))],
                    stderr=subprocess.STDOUT)
        line = '{:=^60}'.format('')
        print(line)
        print(line)
        print('Orly ready to demo')
        print('Go to http://localhost:8000/console')
