...
void AppendMerge(TJson &target, TJson &&addin) {
  ...
  if (target.GetKind() != addin.GetKind()) {
    THROWER(TInvalidValue) << "Unable to merge addin into target. Can't merge non-identical types "
                           << target.GetKind() << " and " << addin.GetKind();
  }
...
void TConfig::AddMixin(const std::string &name,
                       const TCoreDirs &core_dirs,
                       bool is_optional,
                       bool is_system) {
